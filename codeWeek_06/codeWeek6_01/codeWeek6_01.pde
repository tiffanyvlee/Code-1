...
lastArray[1] = 40;
lastArray[4] = 800;
lastArray[50] = lastArray[4];
lastArray[72] = lastArray[4] - 200;
...
for (int j = 0; j < otherArray.length; j++) {
  otherArray[j] = 2;
println("otherArray at position " + j + " is: " + otherArray[j]);
}

for (int k = 0; k < lastArray.length; k++) {
  lastArray[k] = 5;
  println("lastArray at position " + k + " is: " + lastArray[k]);
}